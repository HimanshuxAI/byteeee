import {
  chunk_array_exports
} from "./chunk-TVL7GCHT.js";
import {
  StructuredTool,
  external_exports,
  tools_exports
} from "./chunk-55PA3PR3.js";
import {
  LLM,
  llms_exports,
  openai_tools_exports
} from "./chunk-OIMFVDIY.js";
import {
  Embeddings,
  embeddings_exports
} from "./chunk-WMVVLWQS.js";
import {
  BaseChatModel,
  ...
  runnables_exports,
  types_exports
} from "./chunk-ESYLC4XF.js";
import {
  ChatPromptValue,
  ImagePromptValue,
  StringPromptValue,
  base_exports as base_exports3,
  caches_exports,
  hash_exports,
  prompt_values_exports,
  sha256,
  tiktoken_exports
} from "./chunk-M6OKXC7T.js";
import "./chunk-YN7YRZI4.js";
import {
  messages_exports
} from "./chunk-M2265FND.js";
import {
  AIMessage,
  AIMessageChunk,
  ...
  unescapeValue,
  v5_default
} from "./chunk-5RKU63UL.js";
import "./chunk-WL72BOWM.js";
...
function uuid5(name, namespace) {
  ...
  return v5_default2(name, new Uint8Array(namespaceBytes));
}
...
var base_exports4 = {};
__export(base_exports4, { BaseDocumentLoader: () => BaseDocumentLoader });
...
function patchCheckpointMap(config, metadata) {
  ...
  if (Object.keys(parents).length > 0) return patchConfigurable2(config, { [CONFIG_KEY_CHECKPOINT_MAP]: {
    ...parents,
  ...
  for (const { id, name, input, config, triggers, writes } of tasks) {
    ...
    yield {
      ...
      interrupts: writes.filter(([writeId, n2]) => {
        return writeId === id && n2 === INTERRUPT;
      }).map(([, v]) => {
        return v;
      })
    };
...
function tasksWithWrites(tasks, pendingWrites, states, outputKeys) {
  return tasks.map((task) => {
    const error = pendingWrites.find(([id, n2]) => id === task.id && n2 === ERROR)?.[2];
    const interrupts = pendingWrites.filter(([id, n2]) => id === task.id && n2 === INTERRUPT).map(([, , v]) => v);
    const result = (() => {
      ...
      if (typeof outputKeys === "string") return pendingWrites.find(([tid, n2]) => tid === task.id && n2 === outputKeys)?.[2];
...
var PregelLoop = class PregelLoop2 {
  ...
  static async initialize(params) {
    ...
    if (config.configurable && scratchpad) {
      if (scratchpad.subgraphCounter > 0) config = patchConfigurable2(config, { [CONFIG_KEY_CHECKPOINT_NS]: [config.configurable[CONFIG_KEY_CHECKPOINT_NS], scratchpad.subgraphCounter.toString()].join(CHECKPOINT_NAMESPACE_SEPARATOR) });
      scratchpad.subgraphCounter += 1;
    }
    const isNested = CONFIG_KEY_READ in (config.configurable ?? {});
    if (!isNested && config.configurable?.checkpoint_ns !== void 0 && config.configurable?.checkpoint_ns !== "") config = patchConfigurable2(config, {
      checkpoint_ns: "",
      checkpoint_id: void 0
    });
    let checkpointConfig = config;
    if (config.configurable?.[CONFIG_KEY_CHECKPOINT_MAP] !== void 0 && config.configurable?.[CONFIG_KEY_CHECKPOINT_MAP]?.[config.configurable?.checkpoint_ns]) checkpointConfig = patchConfigurable2(config, { checkpoint_id: config.configurable[CONFIG_KEY_CHECKPOINT_MAP][config.configurable?.checkpoint_ns] });
  ...
  */
  putWrites(taskId, writes) {
    ...
    if (hasUntrackedChannels) writesToSave = writesCopy.filter(([c]) => {
      ...
      if (c === TASKS && _isSend(v)) return [c, sanitizeUntrackedValuesInSend(v, this.channels)];
      return [c, v];
    });
    ...
    if (this.durability !== "exit" && this.checkpointer != null) this.checkpointerPromises.push(this.checkpointer.putWrites(config, writesToSave, taskId));
    ...
    if (task !== void 0) {
      ...
      if (writes.length > 0) {
        if (writes[0][0] === INTERRUPT) {
          if (task.path?.[0] === PUSH && task.path?.[task.path.length - 1] === true) return;
  ...
  */
  ...
  async acceptPush(task, writeIdx, call2) {
...
async function _runWithRetry(pregelTask, retryPolicy, configurable, signal) {
  ...
  if (configurable) config = patchConfigurable2(config, configurable);
  config = {
    ...config,
    signal
  };
  ...
  _commit(task, error) {
    if (error !== void 0) if (isGraphInterrupt(error)) {
      if (error.interrupts.length) {
        const interrupts = error.interrupts.map((interrupt2) => [INTERRUPT, interrupt2]);
...
var CompiledGraph = class extends Pregel {
  ...
  */
  async getGraphAsync(config) {
    ...
    const startNodes = { [START]: graph.addNode({ schema: external_exports.any() }, START) };
    const endNodes = {};
    let subgraphs = {};
    ...
    function addEdge(start, end, label, conditional = false) {
      if (end === END && endNodes[END] === void 0) endNodes[END] = graph.addNode({ schema: external_exports.any() }, END);
      ...
      if (xray) {
        ...
        if (Object.keys(drawableSubgraph.nodes).length > 1) {
          let _isRunnableInterface = function(thing) {
            ...
            if (id !== void 0 && !validate_default2(id)) return id;
...
var ToolNode = class extends RunnableCallable {
  ...
  async runTool(call2, config) {
    const tool = this.tools.find((tool$1) => tool$1.name === call2.name);
    try {
      if (tool === void 0) throw new Error(`Tool "${call2.name}" not found.`);
 ...
 Please fix your mistakes.`,
  ...
  async run(input, config) {
    ...
    if (isSendInput(input)) outputs = [await this.runTool(input.lg_tool_call, config)];
      ...
      outputs = await Promise.all(aiMessage.tool_calls?.filter((call2) => call2.id == null || !toolMessageIds.has(call2.id)).map((call2) => this.runTool(call2, config)) ?? []);
    }
