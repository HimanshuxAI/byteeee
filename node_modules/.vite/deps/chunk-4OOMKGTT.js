...
// node_modules/mermaid/dist/chunks/mermaid.esm.min/chunk-EFRVIJHI.mjs
var nt = q(f(), 1);
var ct = "​";
...
var ft = m(function(e, t) {
  let r = rt(e, /(?:init\b)|(?:initialize\b)/), n = {};
  if (Array.isArray(r)) {
    let s = r.map((u) => u.args);
    Jt(s), n = v(n, [...s]);
  } else n = r.args;
  if (!n) return;
  let i = os(e, t), o = "config";
...
`, "ig");
    e = e.trim().replace(r, "").replace(/'/gm, '"'), pt.debug(`Detecting diagram directive${t !== null ? " type:" + t : ""} based on the text:${e}`);
    let n, i = [];
    for (; (n = Ti.exec(e)) !== null; ) if (n.index === Ti.lastIndex && Ti.lastIndex++, n && !t || t && n[1]?.match(t) || t && n[2]?.match(t)) {
    ...
    return pt.error(`ERROR: ${r.message} - Unable to parse directive type: '${t}' based on the text: '${e}'`), { type: void 0, args: null };
  }
}, "detectDirective");
var zt = m(function(e) {
  return e.replace(Ti, "");
...
m(ht, "formatUrl");
var mt = m((e, ...t) => {
  let r = e.split("."), n = r.length - 1, i = r[n], o = window;
  ...
  for (let i of e) {
    if (r) {
      ...
      if (o < n) n -= o;
        ...
        if (s > 0 && s < 1) return { x: tt((1 - s) * r.x + s * i.x, 5), y: tt((1 - s) * r.y + s * i.y, 5) };
      }
    }
    r = i;
  }
...
var yt = m((e, t, r) => {
  pt.info(`our points ${JSON.stringify(t)}`), t[0] !== r && (t = t.reverse());
...
var Tt = m(function(e, t) {
  let r = t.text.replace(hn.lineBreakRegex, " "), [, n] = $(t.fontSize), i = e.append("text");
...
var St = ki((e, t, r) => {
  if (!e || (r = Object.assign({ fontSize: 12, fontWeight: 400, fontFamily: "Arial", joinWith: "<br/>" }, r), hn.lineBreakRegex.test(e))) return e;
  let n = e.split(" ").filter(Boolean), i = [], o = "";
  return n.forEach((s, u) => {
    let c = M(`${s} `, r), l = M(o, r);
    if (c > t) {
      let { hyphenatedStrings: h, remainingWord: f2 } = $t(s, t, "-", r);
      i.push(o, ...h), o = f2;
...
var S = ki((e, t) => {
  ...
  let [, o] = $(r), s = ["sans-serif", n], u = e.split(hn.lineBreakRegex), c = [], l = ia("body");
...
var Jt2 = m((e, t, { counter: r = 0, prefix: n, suffix: i }, o) => o || `${n ? `${n}_` : ""}${e}_${t}_${r}${i ? `_${i}` : ""}`, "getEdgeId");
function Vt(e) {
  return e ?? null;
}
m(Vt, "handleUndefinedAttr");
function Dt(e, t) {
  let r = Math.round(e.x), n = Math.round(e.y), i = t.replace(/(\d+\.\d+)/g, (o) => Math.round(parseFloat(o)).toString());
...
//# sourceMappingURL=chunk-4OOMKGTT.js.map
