/**
 * Stateless HTTP client for the local PolytracerAI backend server.
...
const fetchWithTimeout = async (
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw new Error(`Network error reaching PolytracerAI backend at ${backendUrl}: ${error.message}`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
